...
class Si705x():
	...
	def _send_cmd (self, cmd, num_bytes = 0):
		w = i2c_msg.write(self.SI7050_DEFAULT_ADDRESS, cmd)
		if num_bytes:
			r = i2c_msg.read(self.SI7050_DEFAULT_ADDRESS, num_bytes)
	...
	def reset(self):
		self._send_cmd(self.SI7050_RESET_CMD)
		time.sleep(.01)	# the chip needs a little time to come out of reset
	
	def getTemp(self):
		(msb, lsb, crc) = self._send_cmd(self.SI7050_MEASTEMP_HOLD_CMD, 3)
	...
	def readUserRegister(self):
		return self._send_cmd(self.SI7050_READ_USERREG_CMD, 1)[0]
	...
	def writeUserRegister(self, res1, res0):
		...
		if res0:
			...
			current &= ~res0	
		setting = self.SI7050_WRITE_USERREG_CMD
		setting.append(current)
		w = i2c_msg.write(self.SI7050_DEFAULT_ADDRESS, setting)
		self.bus.i2c_rdwr(w)

	def getSerialNumber(self):
		# Left nibble of SN
		first_access = self._send_cmd(self.SI7050_READ_ID_BYTE1, 8)
		...
		# Right nibble of SN
		second_access = self._send_cmd(self.SI7050_READ_ID_BYTE2, 6)
	...
	def getFirmwareRevision(self):
		return self._send_cmd(self.SI7050_READ_FW_REV, 1)[0]
